...
export async function deleteRadioRegistry(currentSelo: string, password: string) {
  const response = await fetch(
    buildUrl(`/api/radios/registry/${encodeURIComponent(currentSelo.trim())}`),
    {
      method: 'DELETE',
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(errorText || 'Nao foi possivel excluir o cadastro do radio.');
  }
...
export async function uploadRadioConference(payload: RadioConferencePayload) {
  ...
  payload.images
    ...
    .forEach((image) => {
      formData.append('existingImageNames', image.fileName);
    });

  for (const image of payload.images.filter((item) => !item.isExisting)) {
    await appendImageToFormData(formData, 'images', image);
  }
...
async function appendImageToFormData(
  formData: FormData,
  fieldName: string,
  image: RadioConferenceImage,
) {
  if (Platform.OS === 'web') {
    const response = await fetch(image.uri);

    if (!response.ok) {
      throw new Error('Nao foi possivel ler uma das imagens selecionadas.');
    }
...
export async function deleteStockMeasurement(idMedicao: string) {
  const response = await fetch(
    buildUrl(`/api/pcp/medicoes-estoque/${encodeURIComponent(idMedicao.trim())}`),
    {
      method: 'DELETE',
      headers: {
        Accept: 'application/json',
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(errorText || 'Falha ao excluir medicao de estoque.');
  }
}

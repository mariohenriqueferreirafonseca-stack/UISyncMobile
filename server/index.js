...
const radioImagesDirectory =
  process.env.RADIO_IMAGES_DIR ||
  '\\\\192.168.176.19\\Aplicativos\\UISyncMobile\\ConferenciaRadiosImagens';
const almoxImagesDirectory =
  process.env.ALMOX_IMAGES_DIR ||
  path.join(__dirname, 'data', 'almox-images');
...
function buildRadioImageFileName(numeroSelo, date, suffix, extension = 'jpg') {
  const safeNumeroSelo = sanitizeFileNamePart(numeroSelo) || 'radio';
  const datePart = formatFileDate(date);
  const uniqueSuffix = suffix ? `-${sanitizeFileNamePart(suffix)}` : '';
  const safeExtension = sanitizeFileNamePart(extension).replace(/\./g, '') || 'jpg';

  return `${safeNumeroSelo}-${datePart}${uniqueSuffix}.${safeExtension}`;
}
...
async function saveRadioConferenceImages(files, directoryPath, numeroSelo, options = {}) {
  ...
  const serverDate =
    options.createdAt instanceof Date && !Number.isNaN(options.createdAt.getTime())
      ? options.createdAt
      : new Date();
  const stableLocalId = sanitizeFileNamePart(options.localId);

  for (const [index, file] of files.entries()) {
    const timestamp = `${serverDate.getHours().toString().padStart(2, '0')}${serverDate
      .getMinutes()
      .toString()
      .padStart(2, '0')}${serverDate.getSeconds().toString().padStart(2, '0')}${serverDate
      .getMilliseconds()
      .toString()
      .padStart(3, '0')}`;
    const suffix = stableLocalId
      ? `${stableLocalId}-${index + 1}`
      : index === 0
        ? ''
        : `${timestamp}-${index + 1}`;
    const fileName = buildRadioImageFileName(
      numeroSelo,
      serverDate,
      suffix,
...
app.get('/api/radios/search', async (request, response, next) => {
  try {
    ...
    if (canIncludeLatestConference) {
      ...
      selectedColumns.push(
        `CASE
          WHEN latestConference.${escapeIdentifier('LastConferenceAt')} >= DATEADD(day, -@statusWindowDays, SYSUTCDATETIME())
            THEN 'Conferido'
          ELSE 'Pendente'
...
app.get('/api/radios/catalog', async (request, response, next) => {
  try {
    ...
    if (canIncludeLatestConference) {
      ...
      selectedColumns.push(
        `CASE
          WHEN latestConference.${escapeIdentifier('LastConferenceAt')} >= DATEADD(day, -${RADIO_CONFERENCE_STATUS_WINDOW_DAYS}, SYSUTCDATETIME())
            THEN 'Conferido'
          ELSE 'Pendente'
        END AS ${escapeIdentifier('ConferenceStatus')}`,
      );
    ...
    const result = await pool
      ...
      .query(`
        ...
        WHERE ${buildSeloMatchCondition(escapeIdentifier(seloColumnName), '@numeroSelo')}
          AND ${serverDateExpression} >= DATEADD(day, -@days, SYSUTCDATETIME())
      `);

    response.json({
      alreadyCheckedToday: result.recordset.length > 0,
      alreadyCheckedInWindow: result.recordset.length > 0,
      days,
    });
  } catch (error) {
    next(error);
  ...
  try {
    ...
    try {
      for (const row of normalizedRows) {
        ...
        if (!parsedDate || parsedAngle === null || parsedMeasure === null) {
          throw new Error(
            'As linhas de medicao precisam ter data valida e valores numericos em angulo_graus e medida_metros.',
          );
        }

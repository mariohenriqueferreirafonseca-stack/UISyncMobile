...
import {
  ...
  findRadioSelos,
  getLatestRadioConferenceForEdit,
...
export default function RadioConferenceScreen() {
  ...
  const [loadingExistingConference, setLoadingExistingConference] = useState(false);
  const [processingImage, setProcessingImage] = useState(false);
  const [saving, setSaving] = useState(false);
  const [editingConference, setEditingConference] =
    useState<RadioConferenceEditableData | null>(null);
  ...
  const isEditingExistingConference = Boolean(editingConference);

  const applyConferenceDataToForm = useCallback(
    (conference: RadioConferenceEditableData | null) => {
      ...
      setSituacaoGeral(conference?.situacaoGeral || '');
      setObservacao(conference?.observacao || '');
      setImages(conference?.images || []);
    },
    [],
  );
  ...
  useEffect(() => {
    const normalizedSelo = selectedRadio?.RadioSeloComplemento?.trim().toUpperCase();
    const query = numeroSelo.trim().toUpperCase();
    const shouldLoadLatestConference =
      Boolean(normalizedSelo) &&
      normalizedSelo === query &&
      selectedRadio?.ConferenceStatus === 'Conferido';

    if (!normalizedSelo || normalizedSelo !== query) {
      applyConferenceDataToForm(null);
      return;
    }

    if (!shouldLoadLatestConference) {
      applyConferenceDataToForm(null);
      return;
    }
    ...
    void (async () => {
      setLoadingExistingConference(true);

      try {
        const latestConference = await getLatestRadioConferenceForEdit(normalizedSelo);

        if (!active) {
          return;
        }

        applyConferenceDataToForm(latestConference);
      } finally {
        if (active) {
          setLoadingExistingConference(false);
        }
      }
    })();
    ...
    applyConferenceDataToForm,
    numeroSelo,
    selectedRadio?.ConferenceStatus,
    selectedRadio?.RadioSeloComplemento,
  ]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await createRadioConference(
        ...
        },
        {
          ...
          existingLocalId: editingConference?.localId,
          createdAt: editingConference?.createdAt,
        },
      );
    ...
    checklist,
    editingConference?.createdAt,
    editingConference?.localId,
    images,
    numeroSelo,
    observacao,
    resetForm,
        ...
        >
          ...
          >
            ...
            <View style={styles.card}>
              ...
              {loadingExistingConference ? (
                <View style={styles.editingBanner}>
                  <ActivityIndicator size="small" color="#1C6F47" />
                  <Text style={styles.editingBannerText}>
                    Carregando a ultima conferencia...
                  </Text>
                </View>
              ) : null}

              {isEditingExistingConference ? (
                <View style={styles.editingBanner}>
                  <Ionicons name="create-outline" size={18} color="#1C6F47" />
                  <Text style={styles.editingBannerText}>
                    Editando a ultima conferencia deste radio.
                  </Text>
                </View>
              ) : null}

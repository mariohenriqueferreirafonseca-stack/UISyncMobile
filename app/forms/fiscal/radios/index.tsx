...
function sortRadiosForResults(items: RadioListItem[]) {
  return items
    .map((item, index) => ({ item, index }))
    .sort((left, right) => {
      const leftPriority = left.item.ConferenceStatus === 'Pendente' ? 0 : 1;
      const rightPriority = right.item.ConferenceStatus === 'Pendente' ? 0 : 1;
...
export default function RadiosListScreen() {
  ...
  const carregarRadios = useCallback(
    async (params?: { setor?: string; selo?: string }) => {
      ...
      try {
        ...
        setRadios(sortRadiosForResults(response.items));
  ...
  const atualizarBaseOffline = useCallback(async (options?: { reloadList?: boolean }) => {
    ...
    try {
      ...
      if (synced || isOfflineRadioCatalogReady(cache)) {
        ...
        if (synced && options?.reloadList !== false) {
          const response = await carregarRadios();

          setStatusMessage(
            response.items.length > 0
              ? `Base offline atualizada com ${cache.items.length} radios. A lista agora reflete os dados mais recentes do servidor.`
              : 'Base offline atualizada, mas nenhum radio foi encontrado para os filtros informados.',
          );
          return;
        }
  ...
  }, [carregarRadios, carregarSetores]);
    ...
    useCallback(() => {
      ...
      void (async () => {
        ...
        if (!isOfflineRadioCatalogReady(cache)) {
          void atualizarBaseOffline({ reloadList: false });
        } else {
          await carregarSetores();
        }
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <View style={styles.header}>
            ...
            <View style={styles.headerText}>
              <Text style={styles.title}>Conferência de Rádios</Text>
